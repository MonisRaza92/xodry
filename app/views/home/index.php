<!DOCTYPE html>
<html lang="en">

<head>
    <?php include_once __DIR__ . '/../includes/Header.php' ?>
    <title>Xodry Laundry Service - Affordable Laundry & Dry Cleaning in Delhi</title>
 </head>

<body>
    <?php include_once __DIR__ . '/../includes/Navbar.php'?>
    <?php include_once __DIR__ .  '/../includes/login.php'?>
    <?php include_once __DIR__ .  '/../includes/pickupForm.php'?>
    <?php include_once __DIR__ .  '/../includes/userDetailsForm.php'?>
    <?php include_once __DIR__ .  '/partials/heroSection.php'?>
    <?php include_once __DIR__ .  '/partials/whatWeDo.php'?>
    <?php include_once __DIR__ .  '/partials/whyChooseUs.php'?>
    <?php include_once __DIR__ .  '/partials/howItWorks.php'?>
    <?php include_once __DIR__ .  '/partials/pricingSection.php'?>
    <?php include_once __DIR__ .  '/partials/FAQSection.php'?>
    <?php include_once __DIR__ .  '/partials/footerSection.php'?>



    <?php include_once __DIR__ . '/../includes/Footer.php' ?>
    
</body>

</html>